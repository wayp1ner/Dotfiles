...
export default function definePlugin<P extends PluginDef>(p: P & Record<PropertyKey, any>): typeof p & Plugin;
...
export declare function defineDefault<T = any>(value: T): T;
...
type PluginSettingDefaultType<O extends PluginSettingDef> = O extends PluginSettingSelectDef ? (O["options"] extends {
    default?: boolean;
...
export type PluginNative<PluginExports extends Record<string, (event: Electron.IpcMainInvokeEvent, ...args: any[]) => any>> = {
    [key in keyof PluginExports]: PluginExports[key] extends (event: Electron.IpcMainInvokeEvent, ...args: infer Args) => infer Return ? (...args: Args) => Return extends Promise<any> ? Return : Promise<Return> : never;
};
